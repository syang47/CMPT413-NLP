...
class AttentionModule(nn.Module):
    ...
    def calcAlpha(self, decoder_hidden, encoder_out):
        ...
        #Original Scores and Alpha
        #scores = torch.Tensor([seq * [batch * [1]]]).permute(2, 1, 0)
        #alpha = torch.nn.functional.softmax(scores, dim=1)

        #Arceus Scores
        #Correct the attention mechanism, since default had incorrect parameters and formulae
        #score_i = W_enc(h_i_enc)+W_dec(h_dec)
        hEncode = encoder_out.permute(1, 0, 2)
        hDecode = decoder_hidden.permute(1, 0, 2)
        scores = self.W_enc(hEncode) + self.W_dec(hDecode)	

        #alpha = softmax(V_att.tanh(score))
    ...
    def forward(self, decoder_hidden, encoder_out):
        ...
        #Arceus context
        #Remove the said bottlenect of the hidden vector, so the attention works with greater flexibility and range of sequences
...
class Node(object):
    def __init__(self, parent, wordIdx, score, decoder_hidden):
        self.parent = parent
