...
class LSTMTaggerModel(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, chars_vec_dim):
        torch.manual_seed(1)
        ...
        self.lstm = nn.LSTM(embedding_dim + chars_vec_dim, hidden_dim, bidirectional=False)
        # The linear layer that maps from hidden state space to tag space
        self.hidden2tag = nn.Linear(hidden_dim, tagset_size)
    def forward(self, sentence, chars_vec):
        embeds = self.word_embeddings(sentence)
        embeds = torch.cat((embeds, chars_vec), 1)
...
class LSTMTagger:
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim=64):
        ...
        self.model = LSTMTaggerModel(self.embedding_dim, self.hidden_dim, len(self.word_to_ix), len(self.tag_to_ix), chars_vec_dim = 300)
        self.optimizer = optim.SGD(self.model.parameters(), lr=0.01)
    def argmax(self, seq):
        output = []
        with torch.no_grad():
            inputs = prepare_sequence(seq, self.word_to_ix, self.unk)
            chars_vec = char_rep(seq, self.char_to_ix)
            tag_scores = self.model(inputs, chars_vec)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                chars_vec = char_rep(sentence, self.char_to_ix)
                # Step 3. Run our forward pass.
                tag_scores = self.model(sentence_in, chars_vec)
    ...
    def decode(self, inputfile):
        ...
        self.char_to_ix = saved_model['char_to_ix']
        self.model.eval()
        decoder_output = []
